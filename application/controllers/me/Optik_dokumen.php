...
class Optik_dokumen extends CI_Controller
{
    ...
    {
        $data['title'] = "Dokumen Optik ";
        if (userdata('role') != "admin") {
            $data['user'] = $this->admin->getuser2();
            $data['optik_doc2'] = $this->admin->getoptikDoc_namaSurat2();
        } else {
            // $data['user'] = $this->admin->getuser2();
            $data['optik_doc2'] = $this->admin->getoptikDoc_namaSurat();
        }
        ...
        $this->load->view('viewsKreon/optik_doc/data', $data);
    ...
    public function add()
    {
        ...
        if ($this->form_validation->run() == false) {
            $data['title'] = "Tambah data optik medis";
            $data['user'] = $this->admin->getuser2();
            $data['nama_faskes'] = $this->admin->getdataFaskes('Optik');
            $data['nama_surat'] = $this->admin->get('m_nama_surat');
            $this->session->login_session['role'];
            $this->load->view('viewsKreon/templates/header', $data);
            $this->load->view('viewsKreon/templates/sidebar', $data);
            $this->load->view('viewsKreon/templates/topbar', $data);
            $this->load->view('viewsKreon/optik_doc/add', $data);
            $this->load->view('viewsKreon/templates/footer');
        } else {
            $config['upload_path']          = './upload/optik_doc/';
            ...
            if (@$_FILES['dokumen']['name'] != null) {
                if ($this->upload->do_upload('dokumen')) {
                    ...
                    $this->admin->add_optikdoc($post);
                    if ($this->db->affected_rows() > 0) {
                        echo "<script>alert('Data Berhasil Disimpan');
                        window.location='" . site_url('me/optik_dokumen') . "'
                        </script>";
                    ...
                    window.location='" . site_url('me/optik_dokumen/404') . "'
                    </script>";
                }
            } else {
                $this->admin->add_optikdoc($post);
    ...
    public function edit($getId = null)
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data['nama_surat'] = $this->admin->get('m_nama_surat');
            $data['optik_doc'] = $this->admin->get_optikDokumen($id);
            $data['nama_faskes'] = $this->admin->getdataFaskes('Optik');
            $data['user'] = $this->admin->get('user');
            $this->load->view('viewsKreon/templates/header', $data);
            $this->load->view('viewsKreon/templates/sidebar', $data);
            $this->load->view('viewsKreon/templates/topbar', $data);
            $this->load->view('viewsKreon/optik_doc/edit', $data);
            $this->load->view('viewsKreon/templates/footer');
        } else {
            $config['upload_path']          = './upload/optik_doc/';
            $config['allowed_types']        = 'docx|pdf|';
            ...
            if ($_FILES['editdokumen']['name'] != null) {
                ...
                if (@$_FILES['editdokumen']['name'] != null) {
                    if ($this->upload->do_upload('editdokumen')) {
                        $post['dokumen'] = $this->upload->data('file_name');
                        $this->admin->update_optikdoc($post);
                        if ($this->db->affected_rows() > 0) {
                    ...
                    if ($this->db->affected_rows() > 0) {
                        ...
                        $update = $this->admin->update('optik_doc', 'id_optik_doc', $id, $input);
                        if ($update) {
                            set_pesan('data berhasil');
                            redirect('me/optik_dokumen');
                        }
                    }
                }
    ...
    {
        $id = encode_php_tags($getId);
        if ($this->admin->delete('optik_doc', 'id_optik_doc', $id)) {
    ...
    public function status($getId)
    {
        ...
        $id = encode_php_tags($getId);
        $this->admin->updateStatusOptikDOC($id, $status, $komentar);
        if ($this->db->affected_rows() > 0) {
            echo "<script>alert('Data Berhasil Disimpan');
            window.location='" . site_url('me/optik_dokumen') . "'
            </script>";
    ...
    public function stj($getId = null, $id_tele)
    {
        ...
        $data = $this->admin->get_optikDokumen($getId);
