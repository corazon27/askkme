...
class RS_tm extends CI_Controller
{
    ...
    {
        $data['title'] = "Tenaga Medis Rumah Sakit";
        if (userdata('role') != "admin") {
            $data['user'] = $this->admin->getuser2();
            $data['kreon_tm2'] = $this->admin->getKreonTenagaMedis_namaSurat2();
        } else {
            $data['kreon_tm2'] = $this->admin->getKreonTenagaMedis_namaSurat();
        }
        ...
        $this->load->view('viewsKreon/rs_medis/data', $data);
    ...
    public function add()
    {
        ...
        if ($this->form_validation->run() == false) {
            $data['title'] = "Tambah Data Tenaga Medis Rumah Sakit";
            $data['user'] = $this->admin->getuser2();
            ...
            $this->load->view('viewsKreon/templates/sidebar', $data);
            $this->load->view('viewsKreon/templates/topbar', $data);
            $this->load->view('viewsKreon/rs_medis/add', $data);
            $this->load->view('viewsKreon/templates/footer');
        } else {
            $config['upload_path']          = './upload/rs/';
            ...
            if ($this->db->affected_rows() > 0) {
                ...
                window.location='" . site_url('me/rs_tm') . "'
                </script>";
            } else {
                $this->admin->add($post);
                if ($this->db->affected_rows() > 0) {
    ...
    public function edit($getId = null)
    {
        ...
        if ($this->form_validation->run() == false) {
            $data['title'] = "Edit Data Tenaga Medis Rumah Sakit";
            $data['nama_surat'] = $this->admin->get('m_nama_surat');
            $data['kreon_tm'] = $this->admin->get_kreonMedis($id);
            $data['m_nama_faskes'] = $this->admin->getdataFaskes('Rumah Sakit');
            $data['jns'] = $this->admin->get('m_jns_tng_mds');
            $this->load->view('viewsKreon/templates/header', $data);
            $this->load->view('viewsKreon/templates/sidebar', $data);
            $this->load->view('viewsKreon/templates/topbar', $data);
            $this->load->view('viewsKreon/rs_medis/edit', $data);
            $this->load->view('viewsKreon/templates/footer');
        } else {
            $config['upload_path']          = './upload/rs/';
            $config['allowed_types']        = 'docx|pdf|';
    ...
    {
        $id = encode_php_tags($getId);
        if ($this->admin->delete('kreon_tm', 'id_kreon_tm', $id)) {
    ...
    public function status($getId)
    {
        ...
        $id = encode_php_tags($getId);
        $this->admin->updateStatusKreonTM($id, $status, $komentar);
        if ($this->db->affected_rows() > 0) {
            echo "<script>alert('Data Berhasil Disimpan');
            window.location='" . site_url('me/rs_tm') . "'
            </script>";
    ...
    public function stj($getId = null, $id_tele)
    {
        ...
        $data = $this->admin->get_kreonMedis($getId);
