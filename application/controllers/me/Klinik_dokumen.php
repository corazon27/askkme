...
class Klinik_dokumen extends CI_Controller
{
    ...
    {
        $data['title'] = "Dokumen Klinik Utama";
        if (userdata('role') != "admin") {
            $data['user'] = $this->admin->getuser2();
            $data['klinik_doc2'] = $this->admin->getKlinikDoc_namaSurat2();
        } else {
            // $data['user'] = $this->admin->getuser2();
            $data['klinik_doc2'] = $this->admin->getKlinikDoc_namaSurat();
        }
        ...
        $this->load->view('viewsKreon/klinik_doc/data', $data);
    ...
    public function add()
    {
        ...
        if ($this->form_validation->run() == false) {
            $data['title'] = "Tambah Data Dokumen Klinik Utama";
            $data['user'] = $this->admin->getuser2();
            ...
            $this->load->view('viewsKreon/templates/sidebar', $data);
            $this->load->view('viewsKreon/templates/topbar', $data);
            $this->load->view('viewsKreon/klinik_doc/add', $data);
            $this->load->view('viewsKreon/templates/footer');
        } else {
            $config['upload_path']          = './upload/klinik_doc/';
            ...
            if (@$_FILES['dokumen']['name'] != null) {
                if ($this->upload->do_upload('dokumen')) {
                    ...
                    $this->admin->add_klinikdoc($post);
                    if ($this->db->affected_rows() > 0) {
                        set_pesan(' data berhasil disimpan');
                        echo "<script>alert('Data Berhasil Disimpan');
                        window.location='" . site_url('me/klinik_dokumen') . "'
                        </script>";
                    }
                } else {
                    $error = $this->upload->display_errors();
                    echo "<script>alert($error);
                    window.location='" . site_url('me/klinik_dokumen/404') . "'
                    </script>";
                }
            } else {
                $this->admin->add_klinikdoc($post);
    ...
    public function edit($getId = null)
    {
        ...
        if ($this->form_validation->run() == false) {
            $data['title'] = "Edit Data Dokumen Klinik Utama";
            $data['nama_surat'] = $this->admin->get('m_nama_surat');
            $data['nama_faskes'] = $this->admin->getdataFaskes('Klinik Utama');
            $data['klinik_doc'] = $this->admin->get_klinikDokumen($id);
            $data['user'] = $this->admin->get('user');
            $this->load->view('viewsKreon/templates/header', $data);
            $this->load->view('viewsKreon/templates/sidebar', $data);
            $this->load->view('viewsKreon/templates/topbar', $data);
            $this->load->view('viewsKreon/klinik_doc/edit', $data);
            $this->load->view('viewsKreon/templates/footer');
        } else {
            $config['upload_path']          = './upload/klinik_doc/';
            $config['allowed_types']        = 'docx|pdf|';
            ...
            if ($_FILES['editdokumen']['name'] != null) {
                ...
                if (@$_FILES['editdokumen']['name'] != null) {
                    if ($this->upload->do_upload('editdokumen')) {
                        $post['dokumen'] = $this->upload->data('file_name');
                        $this->admin->update_klinikdoc($post);
                        if ($this->db->affected_rows() > 0) {
                    ...
                    if ($this->db->affected_rows() > 0) {
                        ...
                        $update = $this->admin->update('klinik_doc', 'id_klinik_doc', $id, $input);
                        if ($update) {
                            set_pesan('data berhasil');
                            redirect('me/klinik_dokumen');
                        }
                    }
                }
    ...
    {
        $id = encode_php_tags($getId);
        if ($this->admin->delete('klinik_doc', 'id_klinik_doc', $id)) {
    ...
    public function status($getId)
    {
        ...
        $id = encode_php_tags($getId);
        $this->admin->updateStatusKlinikDOC($id, $status, $komentar);
        if ($this->db->affected_rows() > 0) {
            echo "<script>alert('Data Berhasil Disimpan');
            window.location='" . site_url('me/klinik_dokumen') . "'
            </script>";
    ...
    public function stj($getId = null, $id_tele)
    {
        ...
        $data = $this->admin->get_klinikDokumen($getId);
