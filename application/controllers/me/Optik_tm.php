...
class Optik_tm extends CI_Controller
{
    ...
    {
        $data['title'] = "Tenaga Medis Optik";
        if (userdata('role') != "admin") {
            $data['user'] = $this->admin->getuser2();
            $data['optik_tm2'] = $this->admin->getoptikTenagaMedis_namaSurat2();
        } else {
            $data['optik_tm2'] = $this->admin->getoptikTenagaMedis_namaSurat();
        }
        ...
        $this->load->view('viewsKreon/optik_medis/data', $data);
    ...
    public function add()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->load->view('viewsKreon/templates/sidebar', $data);
            $this->load->view('viewsKreon/templates/topbar', $data);
            $this->load->view('viewsKreon/optik_medis/add', $data);
            $this->load->view('viewsKreon/templates/footer');
        } else {
            $config['upload_path']          = './upload/optik/';
            ...
            $this->admin->add_optik($post);
            if ($this->db->affected_rows() > 0) {
                echo "<script>alert('Data Berhasil Disimpan');
                window.location='" . site_url('me/optik_tm') . "'
                </script>";
            } else {
                $this->admin->add_optik($post);
                if ($this->db->affected_rows() > 0) {
    ...
    public function edit($getId = null)
    {
        ...
        if ($this->form_validation->run() == false) {
            $data['title'] = "Edit Data Optik";
            $data['nama_surat'] = $this->admin->get('m_nama_surat');
            $data['optik_tm'] = $this->admin->get_optikMedis($id);
            $data['nama_faskes'] = $this->admin->getdataFaskes('Optik');
            $data['jns'] = $this->admin->get('m_jns_tng_mds');
            $this->load->view('viewsKreon/templates/header', $data);
            $this->load->view('viewsKreon/templates/sidebar', $data);
            $this->load->view('viewsKreon/templates/topbar', $data);
            $this->load->view('viewsKreon/optik_medis/edit', $data);
            $this->load->view('viewsKreon/templates/footer');
        } else {
            $config['upload_path']          = './upload/optik/';
            $config['allowed_types']        = 'docx|pdf|';
            ...
            if ($_FILES['editdokumen2']['name'] != null) {
                ...
                $post['dokumen2'] = $this->input->post('dokumen2');
                $this->admin->update_optik($post);
                if ($this->db->affected_rows() > 0) {
                    set_pesan('data berhasil diupdate.');
                    echo "<script>alert('Data Berhasil Disimpan');
    ...
    {
        $id = encode_php_tags($getId);
        if ($this->admin->delete('optik_tm', 'id_optik_tm', $id)) {
            set_pesan('data berhasil dihapus.');
        } else {
            set_pesan('data gagal dihapus.', false);
        }
        redirect('me/optik_tm');
    }
    ...
    public function status($getId)
    {
        ...
        $id = encode_php_tags($getId);
        $this->admin->updateStatusOptikTM($id, $status, $komentar);
        if ($this->db->affected_rows() > 0) {
            echo "<script>alert('Data Berhasil Disimpan');
            window.location='" . site_url('me/optik_tm') . "'
            </script>";
    ...
    public function stj($getId = null, $id_tele)
    {
        ...
        $data = $this->admin->get_optikMedis($getId);
