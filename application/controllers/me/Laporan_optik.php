...
class Laporan_optik extends CI_Controller
{
    ...
    {
        $this->form_validation->set_rules('transaksi', 'Laporan', 'required|in_list[optik_tm,optik_doc]');
        $this->form_validation->set_rules('tanggal', 'Periode Tanggal', 'required');

        if ($this->form_validation->run() == false) {
            $data['title'] = "Laporan Kredensialing Optik";
            ...
            $this->load->view('viewsKreon/laporan/formoptik', $data);
            ...
            $query = '';
            if ($table == 'optik_tm') {
                $query = $this->admin->getLaporanKredensialingOptik(null, null, ['mulai' => $mulai, 'akhir' => $akhir]);
            } else {
                $query = $this->admin->getLaporanDokumenOptik(null, null, ['mulai' => $mulai, 'akhir' => $akhir]);
            }
    ...
    private function _cetak($data, $table_, $tanggal)
    {
        ...
        $table = $table_ == 'optik_tm' ? 'Kredensialing Tenaga Medis Optik' : 'Dokumen Kredensialing Optik';
