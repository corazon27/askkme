...
class Kredensialing extends CI_Controller
{
    ...
    public function addKredensialing()
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            window.location='" . site_url('sekagus/hasil_kredensialing') . "'
            </script>";
        }
        set_pesan('data berhasil disimpan.');
        redirect('sekagus/hasil_kredensialing/');
    }
    ...
    public function update($id)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            echo "<script>alert('Data Berhasil Disimpan');
            window.location='" . site_url('hasil_kredensialing') . "'
            </script>";
        }
        set_pesan('data berhasil disimpan.');
        redirect('hasil_kredensialing');

        // $update = $this->admin->update('checkbox', 'id_checkbox', $id, $data);
    }

    // public function statusSelf()
    // {
    // }
}
