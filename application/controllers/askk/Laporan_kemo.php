...
class Laporan_kemo extends CI_Controller
{
    ...
    {
        $this->form_validation->set_rules('transaksi', 'Laporan', 'required|in_list[checkbox_kemo]');
        $this->form_validation->set_rules('tanggal', 'Periode Tanggal', 'required');

        if ($this->form_validation->run() == false) {
            $data['title'] = "Laporan Kredensialing Kemoterapi";
            $this->load->view('templates/header', $data);
            $this->load->view('templates/sidebar', $data);
            $this->load->view('templates/topbar', $data);
            $this->load->view('laporan/formkemo', $data);
            ...
            $query = $this->admin->getLaporanKredensialingKemo(null, null, ['mulai' => $mulai, 'akhir' => $akhir]);
    ...
    private function _cetak($data, $table, $tanggal)
    {
        ...
        $table = 'Kredensialing Poli Kemoterapi';
        ...
        $pdf->Cell(85, 7, 'Nama Faskes', 1, 0, 'C');
        $pdf->Cell(40, 7, 'Nama Poli', 1, 0, 'C');
        $pdf->Cell(45, 7, 'Tgl Self Assesment', 1, 0, 'C');
        $pdf->Cell(45, 7, 'Tgl Kredensialing', 1, 0, 'C');
        ...
        foreach ($data as $d) {
            ...
            $pdf->Cell(15, 7, $no++ . '.', 1, 0, 'C');
            $pdf->Cell(85, 7, $d['nama_faskes'], 1, 0, 'C');
            $pdf->Cell(40, 7, $d['nama_poli'], 1, 0, 'C');
            $pdf->Cell(45, 7, $d['tgl'], 1, 0, 'C');
            $pdf->Cell(45, 7, $d['tgKreon'], 1, 0, 'C');
